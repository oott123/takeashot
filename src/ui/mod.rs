...
impl EguiState {
    ...
    /// Returns any toolbar changes detected this frame.
    ///
    ...
    pub fn run_ui(
        ...
        display: &toolbar::ToolbarDisplay,
        selection_rect: Option<crate::geom::Rect>,
        output_pos: (i32, i32),
        output_size: (u32, u32),
    ) -> toolbar::ToolbarChanges {
        ...
        let full_output = self.ctx.run_ui(self.raw_input.take(), |ctx| {
            toolbar::draw_toolbar(ctx, active_tool, display, selection_rect, output_pos, output_size)
        });
        ...
        toolbar::ToolbarChanges {
            tool: toolbar::take_tool_change(&self.ctx),
            color: toolbar::take_color_change(&self.ctx),
            stroke_width: toolbar::take_stroke_width_change(&self.ctx),
            filled: toolbar::take_fill_change(&self.ctx),
            blur_passes: toolbar::take_blur_passes_change(&self.ctx),
        }
    }
